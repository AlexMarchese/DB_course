...
df = pd.read_excel("emb_videos.xlsx")
...
for iteration in range(3):
    ...
    for row in df.itertuples():
        ...
        if row.embeddings != False: # this is the case, if there is already an embedding from before
            continue # skipping to the next
        ...
        try:
            ...
            # print("worked!")
            worked += 1
            time.sleep(0.003) # to make sure no number of requests per minute is exceeded
        except:
...
# # saving the df
df.to_excel('red_emb_videos.xlsx', index=False)
